...
/**
 * TEMPLATE C -- two-anchor gap-spanning feature, the pattern point/multi-anchor placers
 * ({@link BoulderFeaturePlacer}, {@link HoodooClusterFeaturePlacer}) can't cover: a structure
 * that has to confirm there's an actual gap to bridge before it commits to anything. Copy this
 * shape for natural bridges, sea arches between two headlands, or any other "two roughly
 * equal-height anchors with a saddle between them" landmark.
 *
 ...
 *     finds one. Nearly every candidate site rejects here -- that's expected, arches should be
 *     rare and only where the terrain actually supports them.</li>
 *     <li><b>The overall geometry (anchor heights, spring height, arc apex) is derived entirely
 *     from the diffusion raster, not from {@code chunk.getHeight}.</b> An arch can span across a
 ...
 *     only an approximation of it. A local refinement worth trying: when a leg's column happens to
 *     fall inside the current chunk, blend its raster-based ground Y toward
 *     {@code chunk.getHeight(...)} instead of using the raster value outright.</li>
 * </ul>
 */
public final class ArchFeaturePlacer implements SurfaceFeaturePlacer {
    private static final long SALT = 0xA24C7E57L;
    ...
    private void buildArch(ChunkAccess chunk, long siteSeed, int ax, int az, float elevA,
        ...
        int apexRise = Math.max(2, Math.min(5, Math.round(span * 0.3f)));
    ...
    private void placeLeg(ChunkAccess chunk, Heightmap worldSurface, Heightmap motionBlocking, long siteSeed,
        ...
        int loY = Math.max(groundY + 1, chunk.getMinY());
        int hiY = Math.min(springY, chunk.getMaxY());
    ...
    private void placeIfAir(ChunkAccess chunk, Heightmap worldSurface, Heightmap motionBlocking, long siteSeed,
        ...
        if (worldY < chunk.getMinY() || worldY > chunk.getMaxY()) return;
    ...
    private static BlockState blockFor(long siteSeed, int x, int y, int z) {
        ...
        if (roll < 0.15f) return Blocks.MOSSY_COBBLESTONE.defaultBlockState();
        if (roll < 0.35f) return Blocks.COBBLESTONE.defaultBlockState();
